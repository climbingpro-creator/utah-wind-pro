import { AlertTriangle, CheckCircle, Radio, TrendingUp, Gauge, Eye, Thermometer, Wind, CloudRain } from 'lucide-react';
...
export default function SignalConvergence({ intelligence }) {
  ...
  const { regime, regimeConfidence, signals, conflicts, convergenceScore, adjustedThermalProbability, narrative } = intelligence;
      ...
      <div className="space-y-1">
        ...
        <div className="grid grid-cols-2 gap-1">
          {signals.map((signal, i) => {
            ...
            const typeColor = signal.type === 'bullish' || signal.type === 'thermal_favorable'
              ? 'text-emerald-400'
              : signal.type === 'bearish' || signal.type === 'north_flow'
                ? 'text-red-400'
              ...
              <div key={i} className="flex items-center gap-1.5 text-[10px] px-1.5 py-1 rounded bg-[var(--bg-secondary)]">
                ...
                <div className="ml-auto flex-shrink-0">
                  <div className="w-6 h-1 bg-[var(--border-subtle)] rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${signal.type === 'bearish' || signal.type === 'north_flow' ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${signal.strength * 100}%` }} />
                  </div>
                </div>
              </div>
            );
