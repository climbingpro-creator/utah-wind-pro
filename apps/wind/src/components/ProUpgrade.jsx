...
import { X, Check, Lock, Wind, Zap, Bell, Fish, Anchor, Map, BarChart3, Crown, Sparkles, Eye, EyeOff } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useTheme } from '../context/ThemeContext';

const FEATURES = [
  { icon: Wind,     label: 'Basic wind speed & direction',       free: true,  pro: true },
  { icon: Map,      label: 'Utah Lake conditions',               free: true,  pro: true },
  { icon: BarChart3,label: '3-hour wind history',                free: true,  pro: true },
  { icon: Zap,      label: 'Activity score (good / ok / bad)',   free: true,  pro: true },
  { icon: Map,      label: 'All lakes (Jordanelle, Deer Creek, Willard Bay...)', free: false, pro: true },
  { icon: Zap,      label: 'Thermal prediction engine & ETA',    free: false, pro: true },
  { icon: BarChart3,label: 'Pressure gradient & confidence %',   free: false, pro: true },
  { icon: Wind,     label: '24-hour history & trend analysis',   free: false, pro: true },
  { icon: Fish,     label: 'Fishing intelligence & bite rating', free: false, pro: true },
  { icon: Anchor,   label: 'Boating safety score & wave data',   free: false, pro: true },
  { icon: Bell,     label: 'Unlimited custom wind alerts',       free: false, pro: true },
  { icon: Crown,    label: 'Garmin watch app full access',       free: false, pro: true },
];
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border ${
        ...
        {/* Hero */}
        <div className="relative overflow-hidden rounded-t-2xl px-6 pt-8 pb-6 text-center bg-gradient-to-br from-sky-500 via-cyan-500 to-teal-500">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="space-y-0.5">
            <div className="grid grid-cols-[1fr_52px_52px] gap-1 px-2 pb-2">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider text-center text-sky-500">Pro</span>
            </div>
            {FEATURES.map((f, i) => (
              <div
                key={i}
                className={`grid grid-cols-[1fr_52px_52px] gap-1 items-center px-2 py-1.5 rounded-lg ${
                  !f.free ? (dark ? 'bg-sky-500/5' : 'bg-sky-50/80') : ''
                }`}
              >
                <div className="flex items-center gap-2 min-w-0">
                  <f.icon className={`w-3.5 h-3.5 flex-shrink-0 ${
                    f.free ? (dark ? 'text-slate-500' : 'text-slate-400') : 'text-sky-500'
                  }`} />
                ...
                <form onSubmit={handleEmailPassword} className="space-y-2.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      required
                      minLength={6}
                      className={`w-full px-4 py-2.5 pr-10 rounded-xl text-sm border outline-none focus:ring-2 focus:ring-sky-500/40 ${
                        dark
                          ? 'bg-slate-800 border-slate-700 text-white placeholder:text-slate-500'
                          : 'bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400'
                      }`}
                    ...
                    <button
                      ...
                      className={`absolute right-3 top-1/2 -translate-y-1/2 ${dark ? 'text-slate-400 hover:text-slate-300' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                  ...
                  <button
                    type="submit"
                    disabled={authLoading || !email.trim() || !password.trim() || (authMode === 'signup' && password !== confirmPassword)}
                    className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-all ${
                      dark ? 'bg-slate-800 text-white hover:bg-slate-700 border border-slate-700' : 'bg-slate-900 text-white hover:bg-slate-800'
                    } disabled:opacity-50`}
                  >
