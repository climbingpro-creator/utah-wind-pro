...
import { X, Check, Lock, Fish, Anchor, Map, BarChart3, Crown, Sparkles, Droplets, Waves, Thermometer, Eye, EyeOff } from 'lucide-react';
          ...
          {!user ? (
            <div className="space-y-3">
              ...
              <div className={`relative text-center ${dark ? 'text-slate-600' : 'text-slate-300'}`}>
                ...
                <span className={`relative px-3 text-xs ${dark ? 'bg-slate-900' : 'bg-white'}`}>
                  {authMode === 'magic' ? 'sign in with magic link' : 'sign in to subscribe'}
                </span>
              </div>
              ...
              {authMode === 'magic' ? (
                <form onSubmit={handleMagicLink} className="space-y-2.5">
                  ...
                  <p className={`text-center text-xs ${dark ? 'text-slate-500' : 'text-slate-400'}`}>
                    <button type="button" onClick={() => setAuthMode('signin')} className="text-cyan-500 font-medium">
                ...
                <form onSubmit={handleEmailPassword} className="space-y-2.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      required
                      minLength={6}
                      className={`w-full px-4 py-2.5 pr-10 rounded-xl text-sm border outline-none focus:ring-2 focus:ring-cyan-500/40 ${
                        dark
                          ? 'bg-slate-800 border-slate-700 text-white placeholder:text-slate-500'
                          : 'bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400'
                      }`}
                  ...
                  <div className={`text-center text-xs space-y-1 ${dark ? 'text-slate-500' : 'text-slate-400'}`}>
                    <p>
                      {authMode === 'signin' ? (
                        <>No account? <button type="button" onClick={() => setAuthMode('signup')} className="text-cyan-500 font-medium">Sign up</button></>
                      ) : (
                        <>Have an account? <button type="button" onClick={() => setAuthMode('signin')} className="text-cyan-500 font-medium">Sign in</button></>
                      )}
                    </p>
                    <p>
                      <button type="button" onClick={() => setAuthMode('magic')} className="text-cyan-500/70 font-medium">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-3 rounded-xl font-bold text-sm bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
              >
